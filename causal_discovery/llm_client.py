...
class OpenAIClient(BaseLLMClient):
    def __init__(self, model_id: str = "o3-mini", concurrency: int = 30, base_url: str = None, api_key_env: str = "OPENAI_API_KEY") -> None:
        """
        ...
        # Support custom base_url for OpenAI-compatible APIs like Deepinfra
        client_kwargs = {"api_key": api_key}
        if base_url:
            client_kwargs["base_url"] = base_url
            logging.info(f"Using custom base_url: {base_url}")

        self.client: OpenAI = OpenAI(**client_kwargs)
        self.async_client = AsyncOpenAI(**client_kwargs)
        logging.info(f"Loaded OpenAI-compatible model: {model_id}")

        self.model_id = model_id
        self.semaphore = asyncio.Semaphore(concurrency)
    
    def complete(self, prompt: str, temperature: float = None, frequency_penalty: float = None, presence_penalty: float = None) -> tuple[Optional[str], Optional[dict]]:
