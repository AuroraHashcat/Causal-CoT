...
def extract_causal_skeleton_json(answer: str) -> dict[str, Any]:
    try:
        ...
        # 支持 undirected_edges 字段
        if "undirected_edges" in data:
            result["undirected_edges"] = set(tuple(sorted(edge)) for edge in data["undirected_edges"])
        else:
...
def extract_initial_construct_json(answer: str) -> dict[str, Any]:
    """
    专门用于提取 InitialConstructStage 返回的JSON格式
    
    预期格式:
    {
      "nodes": [{"id": "A", "label": "...", "source": "premise"}, ...],
      "directed_edges": [{"source": "A", "target": "B", "relation_type": "causal"}, ...],
      "undirected_edges": [{"source": "E", "target": "A", "relation_type": "equivalence"}, ...],
    ...
    try:
        ...
        if json_match:
            ...
            if "nodes" in data:
                result["nodes"] = data["nodes"]
                logging.debug(f"extract_initial_construct_json: Found {len(data['nodes'])} nodes")
            else:
                raise ValueError("No nodes found in the JSON data")
            
            # 提取directed_edges - 可选字段
            if "directed_edges" in data:
                result["directed_edges"] = data["directed_edges"]
                logging.debug(f"extract_initial_construct_json: Found {len(data['directed_edges'])} directed edges")
            else:
                result["directed_edges"] = []
                logging.debug("extract_initial_construct_json: No directed_edges found, using empty list")
            
            # 提取undirected_edges - 可选字段
            if "undirected_edges" in data:
                result["undirected_edges"] = data["undirected_edges"]
                logging.debug(f"extract_initial_construct_json: Found {len(data['undirected_edges'])} undirected edges")
            else:
                result["undirected_edges"] = []
                logging.debug("extract_initial_construct_json: No undirected_edges found, using empty list")
            
            # 提取causal_question - 可选字段
            if "causal_question" in data:
                result["causal_question"] = data["causal_question"]
                logging.debug(f"extract_initial_construct_json: Found causal question: {data['causal_question']}")
        ...
        for json_str in json_blocks:
            try:
                ...
                if "nodes" in data and isinstance(data["nodes"], list):
                    # 递归调用处理找到的JSON
                    return extract_initial_construct_json(f"```json\n{json_str}\n```")
                    
            except json.JSONDecodeError:
                continue

        raise ValueError("No valid JSON with nodes found in the answer.")
